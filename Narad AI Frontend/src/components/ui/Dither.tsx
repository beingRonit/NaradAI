...
class RetroEffectImpl extends Effect {
  private _colorNum: THREE.Uniform;
  private _pixelSize: THREE.Uniform;

  constructor() {
    const colorNum = new THREE.Uniform(4.0);
    const pixelSize = new THREE.Uniform(2.0);
  ...
  set colorNum(v: number) {
    this._colorNum.value = v;
  }
  get colorNum() {
    return this._colorNum.value;
  }
  set pixelSize(v: number) {
    this._pixelSize.value = v;
  }
  get pixelSize() {
...
function DitheredWaves({
  ...
  useFrame(({ clock }) => {
    ...
    const w = Math.floor(size.width * dpr);
    const h = Math.floor(size.height * dpr);
    if (u.resolution.value.x !== w || u.resolution.value.y !== h) {
